...
'''
lr = LogisticRegression(penalty = 'l1',solver = 'liblinear',C = 0.12,random_state = 215)
#lr.fit(X_train,Y_train)
lr.fit(feature,label)
print "finish step 5"
'''
predict_X_test = lr.predict_proba(X_test)
source = open("rawlr_validproba.csv","wb")
writer = csv.writer(source)
for each in predict_X_test:
	writer.writerow([each[1]])
source.close()
source = open("validlabel.csv","wb")
writer = csv.writer(source)
for each in Y_test:
...
output = open("result_rawlr.csv","wb")
writer = csv.writer(output)
result = lr.predict_proba(feature)
i = 0
